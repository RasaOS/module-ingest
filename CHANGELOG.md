...
- Added `rasa.identity` ("the RasaOS module for document ingest and similarity-recall memory"); `bin/init` generates `.claude/rasa-identity.md` from it every install + stamps project-owned `.claude/rasa-deployment.md`; ships `/whoami`; CLAUDE.md "Who you are" header.
